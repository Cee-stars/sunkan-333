...
**Gist ID は打たせない。** 貼り口（`#paste-in`）は貼られた文字列を見分け、GitHub のトークンなら
そのまま `connect()` に回す。`connect()` はトークンを保存し、`ensureGist()` で置き場を用意して、
そのまま 1 回同期する。

`ensureGist()` は、手元に Gist ID が無ければ `GET /gists?per_page=100` を見て
`sunkan-data.json` を持つ Gist を探す。無ければ `mydict-data.json` を持つものに相乗りし、
それも無ければ新しく作る。**同じトークンなら 2 台目も同じ置き場に行き着く**ので、
2 台目の設定は「同じトークンを貼る」だけで済む。一覧を読めなかったときは作るほうへ進む。

同期の途中で置き場が 404 になったら（消された場合）、Gist ID を捨てて作り直し、1 回だけやり直す。

**「自動で同期する」の切り替えは持たない。つないである（トークンがある）＝自動**。押し忘れたぶんだけ
端末がずれるため。`sunkan:sync:auto` は古い版のために `'1'` を書くだけで、読んでいない。
...
`tombs` は書くときに**鍵ごと 1 件へまとめる**（`t` / `a` は新しいほうを採る）。手元のぶんと向こうの
ぶんをつなげて渡すので、まとめないと同期のたびに件数が倍になる。
...
放っておいても揃うのが前提なので、入口を広くとる。どれも `sync(silent)` に集まる。

| きっかけ | すること |
| --- | --- |
| 起動（`init`） | 0.8 秒後に 1 回。前回送れなかったぶんもここで片付く |
| 手元が変わった | 3 秒ごとの見張り（`watch`）が指紋で気付き、1.2 秒待って送る |
| 手元が落ち着いている | 45 秒ごとに向こうを取りに行く（開いたまま放っておいても降りてくる） |
| 表に戻った | すぐ 1 回 |
| 裏に回る / ページを離れる | 送り残し（`dirty()`）があれば送る |
| `online` になった | すぐ 1 回 |
| 別タブが `sunkan:` を書いた（`storage`） | 画面を作り直して送る |
| 失敗した | 15 秒 → 1 分 → 5 分 と間を空けてやり直す |

保存は横取りしない（`app.js` / `paraphrase.js` に手を入れない）。代わりに localStorage の
指紋（`fingerprint()`）を見張る。送れたときの指紋は `sunkan:sync:fp` に残すので、
**読み込み直しても「まだ送っていない」が分かる**。裏に回っているタブでは見張らない。

**中身が同じなら PATCH しない**（`payloadOf()` で見分ける）。置き場の履歴を無駄に増やさないため。

`#sync-dialog` は設定ダイアログの中の `#btn-sync-open` から開く（設定は先に閉じる）。
ダイアログは状態で出し分ける: `#sync-setup`（まだトークンが無いとき）と `#sync-live`（つないだあと）。
...
| `sunkan:sync:auto` | `'1'` / `'0'` … 読んでいない（つないである＝自動）。古い版のために書くだけ |
| `sunkan:sync:fp` | 最後に送れたときの指紋。読み込み直しても送り残しが分かる |
