...
- **版を上げるときは `APP_VERSION` と `version.json` の両方**を同じ文字列にする。
  片方だけだと、出るはずのない帯が出続ける。
...
```js
{
  ...
  peers: { 'd1a2b3': { n: 'iPhone・ホーム画面', b: 'build 29 (2026-09-18)', at: 1750000000000 } }
}
```
...
`ensureGist()` は、手元に Gist ID が無ければ `GET /gists?per_page=100` を見て `sunkan-data.json`
を持つ Gist を探す。無ければ `mydict-data.json` を持つものに相乗りし、それも無ければ新しく作る。
**同じトークンなら 2 台目も同じ置き場に行き着く**ので、2 台目の設定は「同じトークンを貼る」だけで済む。
一覧を読めなかったときは作るほうへ進む。

同期の途中で置き場が 404 になったら（消された場合）、Gist ID を捨てて作り直し、1 回だけやり直す。
そのために `ghFetch` は投げるエラーに `status` を添える。

**つないである（トークンがある）＝自動**。`sunkan:sync:auto` は `'0'` のときだけ止める印として読む。
「置き場だけ作って自動が off のまま、黙って何も送らない」状態を作らないため。
...
### 誰が、いつ、どの版で書いたか（`peers`）

**「同期しました」と出ていても、揃っているとは限らない。** もう片方が古い版だと、
書き込みが通らない版（更新に `public` を混ぜていた頃のものは毎回 422 で弾かれる）もあれば、
新しい版が足した項目を落として送り返す版もある。それでも画面は「自動」のままで、
利用者には見分けが付かなかった。

送るたびに、この入れ物の名札を今の時刻で押す（`stampSelf`）。1 件は
`{ n: 名前, b: 版, at: 書いた時刻 }`。鍵は `sunkan:sync:device`（入れ物ごとに 1 つ。
**iOS はホーム画面とブラウザで保存領域が分かれるので、同じ端末でも別の名札になる**）。
名前は UA と `display-mode: standalone` から作る。版は `#app-version` の表示をそのまま読む
（`update.js` と同じ出どころ）。端末ごとに新しいほうを採り、60 日書き込みが無いものは忘れる。

同期の設定を開くと、そのまま一覧で出す。あわせて次を知らせる:

- **ほかの端末の名札が 1 つも無い** … 別の保存先を見ているか、相手が古い版で書けていない
- **版が違う**（14 日以内に書いた端末だけ比べる。使っていない端末を責めても仕方ない）
- **こちらが押した名札が保存先から消えていた** … 古い版が項目ごと落として書いている

どれかに当たると、設定ダイアログのボタンの札が **⚠ 要確認** になる（エラーが出ていればそちらが優先）。
「うまくいかないとき」の ⑤ では、手元の控えではなく**保存先そのもの**を読んで同じことを出す。
...
| きっかけ | すること |
| --- | --- |
| 起動（`init`） | 1.2 秒後に 1 回。前回送れなかったぶんも、置き場がまだ無いぶんもここで片付く |
| 手元が変わった | 3 秒ごとの見張り（`watch`）が指紋で気付き、2 秒待って送る |
| 手元が落ち着いている | 45 秒ごとに向こうを取りに行く（開いたまま放っておいても降りてくる） |
| 表に戻った | すぐ 1 回 |
| 裏に回る / ページを離れる | 送り残し（`dirty()`）があれば送る |
| `online` になった | すぐ 1 回 |
| 別のタブが `sunkan:` を書いた（`storage`） | 送る |
| 失敗した | 15 秒 → 1 分 → 5 分 と間を空けてやり直す |

保存は横取りしない（`app.js` / `paraphrase.js` に手を入れない）。代わりに localStorage の指紋
（`fingerprint()`）を見張る。送れたときの指紋は `sunkan:sync:fp` に残すので、**読み込み直しても
「まだ送っていない」が分かる**。裏に回っているタブでは見張らない。

**中身が同じなら PATCH しない**（`contentSig()` で見分ける。名札は毎回変わるので外す）。
置き場の履歴を無駄に増やさないため。ただし名札は生きている印なので、10 分以上押していなければ
中身が同じでも送り直す。こちらの名札が保存先から消えていたとき（古い版に落とされたとき）も送る。
...
| `sunkan:sync:device` | この入れ物の名札。保存先に誰が書いているかを見分けるためだけに使う |
| `sunkan:sync:peers` | 保存先に書いている端末の控え `{ id: {n,b,at} }`（表示用。正は保存先） |
| `sunkan:sync:stripped` | `'1'` … 押した名札が保存先から消されていた（古い版が書いている印） |
| `sunkan:sync:fp` | 送れたときの指紋。読み込み直しても送り残しが分かる |
