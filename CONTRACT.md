...
**変わった画面だけ作り直す**。受信箱が動いただけで表まで作り直すと、読んでいる行が勝手に閉じる。

**作り直しても、開いていた行／めくった札・見ている位置・並べ方は元に戻す。**
同期は裏で走るので、ここを捨てると読んでいる最中に答えが消える
（`app.js` の `reloadFromStorage`、`paraphrase.js` の `renderCards`）。
...
**受信箱の帯は、先に取り込んだほうにしか出ない。** iOS では同じアプリが複数の入れ物で動くため、
別の入れ物の瞬間英作文が先に「取り込む」を押すと、こちらには帯が出ないまま文だけが同期で入る。
黙って増えると「送ったのに届いていない」に見えるので、同期で足した文が増えたら
`SUNKAN_INBOX.notify()` でその旨を出す（`announceArrivals`）。
