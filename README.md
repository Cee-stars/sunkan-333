...
- **読み上げ** — 英語を音声で確認できます（対応ブラウザのみ）。⚙ 設定の「音声の設定」で**声を選べます**（Samantha など、端末に入っている英語の声から）。「聞いてみる」でその場で確かめられます。
...
- **バックアップを書き出す・読み込む** — ⚙ 設定の「データ」から。JSON ファイル 1 つに、自作セット・足した文・書き換えた文・★・パラフレ帳がまとめて入ります。**機種変更のときは、こちらで書き出して新しい端末で読み込んでください。**
- **CSV で書き出す** — いま開いているセットを Excel などで開ける形に。日本語が化けないようにしてあります。
