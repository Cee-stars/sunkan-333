...
- **iPhone と MacBook で勝手に揃う** — 一度つなげば、自作セット・足した文・★・パラフレ帳が、開いている間ずっと自動で行き来します。押すものはありません。
...
**一度つなげば、あとは何もしなくても揃います。** 直したとき、開いたとき、開いたまま放っておくあいだ（45 秒ごと）
にも自動で行き来します。押すボタンはありません。オフラインだったぶんは、電波が戻った時点でやり直します。
...
（トークンから置き場を探し当てます）。My Dictionary 用に作ったトークンがあれば、それをそのまま使えます。
その場合は My Dictionary と同じ Gist に相乗りします（中では別のファイルに分かれています）。

トークンを打ち込むのが面倒なときは、つないである端末で「**もう片方の端末へ渡すリンクをコピー**」を押し、
もう片方でそのリンクを開くか、同じ欄に貼り付けてください。ホーム画面から開いたアプリには
アドレス欄が無いので、そのときは貼り付けます。**リンクにはトークンが入っているので、人に渡さないでください。**

### つながずに、1 回だけ渡す

登録したくないときは、片方で「**リンクを作ってコピー**」→ もう片方で開く（または貼り付ける）だけでも渡せます。
