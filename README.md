...
- **バックアップを書き出す・読み込む** — ☰ メニューから。JSON ファイル 1 つに、自作セット・足した文・書き換えた文・★・パラフレ帳がまとめて入ります。**機種変更のときは、こちらで書き出して新しい端末で読み込んでください。**
- **CSV で書き出す・読み込む** — いま開いているセットを Excel などで開ける形に。日本語が化けないようにしてあります。Excel で直したものは、そのまま「バックアップを読み込む」で戻せます（JSON と CSV は中身で見分けます）。
...
**バックアップ（☰ → バックアップを書き出す）には入ります**ので、機種変更のときはそちらを使ってください。
