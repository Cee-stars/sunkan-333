...
- **iPhone と MacBook で揃う** — 自作セット・足した文・★・パラフレ帳を、端末どうしで同期できます。
...
自作セット・足した文・★・パラフレ帳を、端末どうしで同期できます。**⚙ 設定 → 同期の設定** から。

サーバーもアカウントも要りません。GitHub の「シークレット Gist」（**URL を知っている人だけが
見られる場所**）を置き場として使います。**完全に非公開というわけではない**点にご注意ください。
...
2. ⚙ 設定 → 同期の設定 → トークンを貼る
3. Gist ID は空のまま「**保存先を新しく作る**」→ ID が入ります
4. 「直したときと、開いたときに自動で揃える」をオンに

### 2 台目

同じトークンと、**1 台目の Gist ID をそのまま**貼り付けるだけです。あとは「いま同期する」を一度押せば揃います。
