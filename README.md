...
- **iPhone と MacBook で勝手に揃う** — 一度つなげば、自作セット・足した文・書き換えた文・★・パラフレ帳が、開いている間ずっと行き来します。押すものはありません。**どの端末がいつ書いたか**も見えるので、「揃っているつもり」になりません。
- **古いままにならない** — 新しい版が出ていると上に帯が出ます。「更新する」を押せば、溜まっている古い一式を捨てて読み直します（⚙ 設定の「最新にする」でも同じ）。
...
自作セット・足した文・★・パラフレ帳を、端末どうしで揃えます。**⚙ 設定 → 同期の設定** から。

**一度つなげば、あとは何もしなくても揃います。** 直したとき、開いたとき、開いたまま放っておく
あいだ（45 秒ごと）にも行き来します。オフラインだったぶんは、電波が戻った時点でやり直します。

### つなぐ（端末ごとに 1 回だけ）

置き場として GitHub の「シークレット Gist」（**URL を知っている人だけが見られる場所**）を使います。
**完全に非公開というわけではない**点にご注意ください。

1. [GitHub でトークンを作る](https://github.com/settings/tokens/new?scopes=gist&description=%E7%9E%AC%E9%96%93%E8%8B%B1%E4%BD%9C%E6%96%87%E3%81%AE%E5%90%8C%E6%9C%9F)
   （権限は **gist** だけで足ります。同期の設定にボタンがあります）
2. できた文字列（`ghp_…`）をコピーして、⚙ 設定 → 同期の設定 → **貼り付けて「つなぐ」**

これだけです。置き場は自動で作られます。

**2 台目も、同じトークンを貼るだけです。** Gist ID を控えて打ち込む必要はありません
（トークンから置き場を探し当てます）。My Dictionary 用のトークンがあれば、そのまま使えます
（その場合は同じ Gist に相乗りします。中では別のファイルに分かれています）。

トークンを打ち込むのが面倒なときは、つないである端末で「**つなぐリンクをコピー**」を押し、
もう片方でそのリンクを開くか、同じ欄に貼り付けてください。ホーム画面から開いたアプリには
アドレス欄が無いので、そのときは貼り付けます。**リンクにはトークンが入っているので、人に渡さないでください。**

### 揃っているか確かめる

同期の設定を開くと、**その置き場に書いている端末**が版と時刻つきで並びます。

```
この保存先に書いている端末
・Mac・ブラウザ … build 29 (2026-09-18) ・たった今（この端末）
・iPhone・ホーム画面 … build 29 (2026-09-18) ・3 分前
```

次のときは、そのまま知らせます（設定の札も **⚠ 要確認** になります）。

- **ほかの端末の名札が無い** … 別の置き場を見ているか、相手が古い版で書けていない
- **版が違う** … 片方が古いままです。古いほうの端末で「更新する」を押してください
- **こちらの名札が消されていた** … 古い版の端末が、知らない項目ごと落として書いています

**古い版は、書き込みそのものが通らないことがあります**（更新に `public` を混ぜていた頃のものは
毎回 422 で弾かれます）。当時は失敗を画面に出していないので、古い端末は「自動」と出したまま
一度も送っていません。両方の版を揃えるのが先です。⚙ 設定の一番下で版を確かめられます。

もっと細かく見るときは「**うまくいかないとき**」を押してください。GitHub に届くか →
トークンが通るか → 置き場が読めるか → 書けるか → 誰が書いているか、を順に試します。

### つながずに、1 回だけ渡す

登録したくないときは、片方で「**リンクを作ってコピー**」→ もう片方で開く（または貼り付ける）。
文が多くてリンクに収まらないときは「**ファイルに書き出す**」→ AirDrop などで送って
「**ファイルを読み込む**」。こちらは大きさの制限がありません。

ただし**これは思い出したときに手で渡す形**で、放っておいて勝手に揃うわけではありません。

> このアプリはサーバーを持たない（GitHub Pages に置いた HTML だけの）作りなので、
> **どこかに置き場を持たないかぎり、裏で勝手に同期する仕組みは作れません。**
> データを預ける先が無いためです。iCloud も、Safari のブックマークやパスワードは同期しますが、
> **アプリのデータ（localStorage）は同期しません。** 上のトークン 1 回ぶんだけは、
> どうしても必要な手間になります。
...
`assets/app.js` の `APP_VERSION` と `version.json` の `build` を**同じ文字列**にして、
`sw.js` の `CACHE` を 1 つ上げます。3 つとも上げないと、更新が届かなかったり、
出るはずのない「新しい版があります」の帯が出続けたりします。
